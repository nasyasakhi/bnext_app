...
import { Slider, getSlider } from "../../../hooks/slider-crud/getSlider";
import Button from "../../ui/button/Button";
import { Table, TableBody, TableCell, TableHeader, TableRow } from "../../ui/table";
import { deleteSliderAPI } from "../../../hooks/slider-crud/deleteSlider";

export default function BasicSliderTable() {
  ...
  function handleNavigateToUpdateForm(ID: number) {
    navigate(`/sliders/update-ads/${ID}`);
  }
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {sliders.map((slider) => (
              <TableRow key={slider.ID}>
                ...
                {/* Menampilkan Category ID dan Name */}
                <TableCell className="px-4 py-3 text-gray-500 text-center text-theme-sm dark:text-gray-400">
                  {slider.category.ID}
                </TableCell>
                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                ...
                <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                  <div className="flex -space-x-2">
                    <div
                      ...
                      onClick={() => openModal(`${baseUrl}/${slider.image_url}`)} // misalnya cuma 1 gambar
                    >
                      <img
                        width={24}
                        height={24}
                        src={`${baseUrl}/${slider.image_url}`} // bukan array, tapi langsung property
                ...
                <TableCell className="px-4 py-3 text-start">
                  <Button variant="solid" color="primary" size="xs"
                    onClick={() => handleNavigateToUpdateForm(slider.ID)}>
                    Update
                  </Button>
                </TableCell>
                <TableCell className="px-4 py-3 text-start">
                  <Button
                    ...
                    disabled={loadingDeleteId === slider.ID}
                  >
                    {loadingDeleteId === slider.ID ? "Menghapus..." : "Delete"}
                  </Button>
                </TableCell>
              </TableRow>
            ))}
